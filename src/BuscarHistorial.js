...
function safeCell(value) {
  ...
  // Si es string y parece ser una fecha en formato DD/MM/YYYY
  if (typeof value === 'string' && value.includes('/') && value.match(/^\d{1,2}\/\d{1,2}\/\d{4}$/)) {
    try {
      const partes = value.split('/');
      if (partes.length === 3) {
        const [dia, mes, año] = partes;
        const fecha = new Date(`${año}-${mes.padStart(2, '0')}-${dia.padStart(2, '0')}`);
      ...
      console.warn('Error procesando fecha:', value, error);
    }
  }
