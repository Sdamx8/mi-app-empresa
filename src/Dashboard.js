...
import InformesTecnicosNew from './InformesTecnicosNew'; // Componente reestructurado
...
const Dashboard = () => {
  ...
  const [informeTecnicoData, setInformeTecnicoData] = useState(null);
  ...
  // Escuchar eventos de generación de informe técnico
  useEffect(() => {
    const handleGenerarInforme = (event) => {
      const { numeroRemision, datosRemision } = event.detail;
      
      // Verificar permisos
      if (!safeHasModuleAccess('informes_tecnicos')) {
        alert('No tienes permisos para acceder al módulo de Informes Técnicos');
        return;
      }

      // Guardar datos y cambiar al módulo
      setInformeTecnicoData({
        numeroRemision,
        datosRemision,
        autoOpen: true
      });
    ...
    window.addEventListener('generarInformeTecnico', handleGenerarInforme);
    
    return () => {
      window.removeEventListener('generarInformeTecnico', handleGenerarInforme);
    };
  }, []);
  ...
  const renderContent = () => {
    ...
    switch (activeModule) {
      ...
      case 'reporte_historial':
        if (!safeHasModuleAccess('reporte_historial')) return <AccessDenied module="Reporte de Historial" />;
        return <ErrorBoundary><ReporteHistorial /></ErrorBoundary>;
      case 'informes_tecnicos':
        if (!safeHasModuleAccess('informes_tecnicos')) return <AccessDenied module="Informes Técnicos" />;
        return (
          <ErrorBoundary>
            <InformesTecnicosNew 
              initialData={informeTecnicoData}
              onDataUsed={() => setInformeTecnicoData(null)}
            />
          </ErrorBoundary>
        );
      default:
  ...
  const modules = [
    ...
    { key: 'reporte_historial', icon: '📈', label: 'Reportes' },
