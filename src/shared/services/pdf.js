...
const crearSeccionServicios = (informe) => [
  // 3. Título de sección con numeración ISO
  { text: '3. VALORACIÓN ECONÓMICA DE SERVICIOS', style: 'sectionTitleISO' },
  // Línea separadora
  {
    canvas: [
      {
    ...
    columns: [
      {
        width: '60%',
        stack: [
          { text: 'Descripción del Servicio:', style: 'fieldLabelISO' },
          { text: informe.tituloTrabajo || 'Servicio no especificado', style: 'fieldValueISO' }
          // ELIMINADO: Campo "Detalles Adicionales" según requerimientos
        ]
      },
      {
        width: '40%',
        stack: [
          ...
          { 
            text: informe.subtotal ? `$ ${Number(informe.subtotal).toLocaleString('es-CO')}` : '$ 0', 
            style: 'fieldValueISO'
          },
          { text: 'Total (incluye IVA):', style: 'fieldLabelISO' },
          { 
            text: informe.total ? `$ ${Number(informe.total).toLocaleString('es-CO')}` : '$ 0', 
            style: 'fieldValueISO', 
            bold: true,
            fontSize: 12,
            color: '#0056A6'
...
const crearSeccionObservaciones = (informe) => {
  ...
  return [
    // 4. Título de sección con numeración ISO
    { text: '4. ACTIVIDADES REALIZADAS', style: 'sectionTitleISO' },
    // Línea separadora
    {
      canvas: [
        {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        const documentDefinitionV7 = {
          ...
          content: [
            // Información del informe (con datos del empleado)
            ...
            ...crearSeccionObservaciones(informe),
