...
export const getAllEmployees = async () => {
  ...
  if (employeeCache && lastFetch && (Date.now() - lastFetch < CACHE_DURATION)) {
    console.log('Usando datos de cache, empleados:', employeeCache.length);
  ...
  try {
    ...
    console.log(`✅ ${empleados.length} empleados obtenidos para correo: ${currentUser.email}`);
    return empleados;
  } catch (error) {
...
export const getEmployeeByEmail = async (email) => {
  ...
  try {
    ...
    console.error('Error al buscar empleado por email:', error);
    return null;
  }
};
