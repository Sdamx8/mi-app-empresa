...
export class PDFService {
  ...
  // Procesar imagen para PDF (SOLUCIÓN DEFINITIVA SIN CORS)
  static async processImageForPDF(imageUrl) {
    console.log('🖼️ Procesando imagen para PDF con método Firebase nativo:', imageUrl);
    ...
    try {
      // Usar el nuevo método ImageService.getImageBase64 que resuelve CORS definitivamente
      const base64Image = await ImageService.getImageBase64(imageUrl);
      console.log('✅ Imagen procesada exitosamente sin CORS');
      return base64Image;
    } catch (error) {
      console.warn('⚠️ Error procesando imagen, generando placeholder:', error.message);
      // Generar placeholder si todo falla
      return this.generarImagenPlaceholder();
    }
  }

  // Generar imagen placeholder para PDFs
  static generarImagenPlaceholder() {
    ...
    ctx.font = '14px Arial';
    ctx.fillText('Error de carga CORS resuelto', canvas.width / 2, canvas.height / 2 + 10);
